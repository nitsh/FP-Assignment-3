...
class FunSetSuite extends FunSuite {
  ...
  test ("intersection contains the common element only" ) {
    new TestSets {
      val s4 = union(s1, s2)
      val s5 = union(s2, s3)
  ...
  test ("diff doesn't contains the common element" ) {
    new TestSets {
      val s4 = union(s1, s2)
      val s5 = union(s2, s3)
      val s = diff(s4, s5)
  ...
  test ("filter will have only 3" ) {
    new TestSets {
      val s4 = union(s1, s2)
      val s = filter(union(s4, s3), x=>x==3)
